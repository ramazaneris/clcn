...
export default class coloredConsole {
   ...
   * @param {string} write Can log to console something with settings
   * @param {string} color Can chose the text color or backgroundcolor
   * @param {string} text Can write console message
   **/
  write(text: string) {
    let txt = text.split("[");
    for (const incode of txt) {
      if (incode.endsWith("]")) {
        let colors = incode.split(":")
...
function txtBlack(text:string) {
  return "\x1b[30m" + text + "\x1b[30m";
}

module.exports = { txtBlack };
